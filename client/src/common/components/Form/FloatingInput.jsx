    ...
    <div>
      ...
      >
        <input
          ...
          ref={refer}
          {...inputProps}
          className={`block shadow-xl px-2.5 pb-2.5 pt-5 w-full text-sm bg-base-200 border-0  ${meta?.touched && meta?.error ? 'border-error' : 'border-primary'
            } appearance-none  focus:border-b-2 focus:outline-none focus:ring-0 focus: ${meta?.touched && meta?.error ? 'border-error' : 'border-primary'
            }/80 peer`}
          placeholder=" "
          autoComplete="off"
        />
        <label
          ...
          className={`absolute text-sm ${meta?.touched && meta?.error ? 'text-error' : ''
            } duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] start-2.5 peer-focus:${meta?.touched && meta?.error ? 'text-error' : 'text-primary'

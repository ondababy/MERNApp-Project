...
import { toast } from 'react-toastify';
import { productApi } from '../product.api';
import { dashUrl } from '../product.api'; 
import { getAltFields, getFields } from '../product.fields';
import { productValidation } from '../product.validation';
import ProductWrapper from './ProductWrapper';
...
const ProductForm = ({ title = 'Product Form', action = 'create' }) => {
  ...
  const fetchBrands = async () => {
    try {
      const response = await fetch('/api/brands'); // Update the URL according to your backend endpoint
      if (!response.ok) throw new Error('Failed to fetch brands');
      const data = await response.json();
      setBrands(data); // Assuming your API returns a list of brands
    } catch (error) {
  ...
  const fetchSuppliers = async () => {
    try {
      const response = await fetch('/api/suppliers'); // Update the URL according to your backend endpoint
      if (!response.ok) throw new Error('Failed to fetch suppliers');
      const data = await response.json();
      setSuppliers(data); // Assuming your API returns a list of suppliers
    } catch (error) {
      toast.error(error.message);
    }
  };
  ...
  return (
    <ProductWrapper title={title} prevUrl="/dashboard/products/table">
      <div className="flex flex-col gap-4 lg:flex-row items-center lg:items-start">
        <div className="container lg:w-1/3 w-96 aspect-square">
          <CarouselComponent
            imageList={
        ...
        <div className="container w-2/3">
          <FormikForm
            ...
            element={({ isSubmitting, values }) => {
              const isFormChanged = !isEqual(initialValues, values);
              const isProcessing = isSubmitting || isFetching;
              const isButtonDisabled = isProcessing || !isFormChanged;
